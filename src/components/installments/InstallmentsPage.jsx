...
export default function InstallmentsPage({
  ...
  const overdueCount = items.filter((c) => computeInstallmentStatus(c) === 'متأخر').length;
  ...
  if (selected) {
    ...
    const descLines = (selected.desc || '').split('\n').map((l) => l.trim()).filter(Boolean);
    ...
    return (
      <>
        ...
        <div className="inst-detail-header">
          <div>
            <div className="section-title" style={{ marginTop: 0 }}>{selected.name}</div>
            {selected.phone && <div className="inst-phone">{selected.phone}</div>}
          </div>
        ...
        <div className="inst-product-box">
          ...
          {descLines.length > 0 ? (
            descLines.map((line, i) => (
              <div key={i} className="inst-product-line">{line}</div>
            ))
          ) : (
        ...
        <div className="inst-detail-meta">
          <span>قسط كل {unitLabel}: <b className="num">{fmt(selected.monthly)}</b></span>
          <span>المقدّم: <b className="num">{fmt(selected.down)}</b></span>
          {selected.firstInstallmentDate && (
            <span>أول قسط: <b>{formatFullDate(selected.firstInstallmentDate)}</b></span>
          )}
        </div>

        <div className="item-actions" style={{ marginBottom: 16 }}>
          {computeInstallmentStatus(selected) !== 'مسدد' && (
            <button className="btn primary" onClick={() => onOpenPaymentForm(selected.id)}>
              ✅ سجّل دفعة
            </button>
          )}
          {selected.paid > 0 && (
            <button className="btn ghost" onClick={() => onUndoPayment(selected.id)}>↩️ تراجع</button>
          )}
          <button className="btn ghost" onClick={() => onOpenDateForm(selected.id)}>📅 تاريخ البداية</button>
        </div>
        ...
        {schedule.length === 0 ? (
          ...
          <div className="ledger-wrap">
            <table className="ledger-table inst-schedule-table">
              ...
              <tbody>
                {schedule.map((row) => (
                  <tr key={row.index} className={row.status === 'متأخر' ? 'inst-row-late' : ''}>
                    <td className="num">{row.index}</td>
                    <td className="ledger-date">{formatShortDate(row.dueDate)}</td>
                    <td className="ledger-date">
                      {row.actualDate ? formatShortDate(row.actualDate) : '—'}
                    </td>
                    <td>
                      {row.status === 'مسدد' ? (
                    ...
                    <td className="inst-settled">
                      {row.settled ? <span className="inst-check">✓</span> : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        <div className="empty">
          {overdueOnly ? 'مفيش متأخرين دلوقتي 👍' : 'مفيش عملاء — دوس ＋ عشان تضيف عميل'}
        </div>
      ) : (
        <div className="ledger-wrap">
              ...
              {filtered.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="num" style={{ color: 'var(--success)' }}>{fmt(paidAmount)}</td>
                    <td
                      className="num"
                      style={{ color: Number(c.remaining) > 0 ? 'var(--danger)' : 'var(--success)' }}
                    >
                      {fmt(c.remaining)}
                    </td>
                    <td>
                      <span className={`pill ${listStatus.cls}`}>{listStatus.label}</span>
                    </td>
                    <td>
                      <button
                        className="mini-btn"
